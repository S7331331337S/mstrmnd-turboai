...
**Production-grade [next-forge](https://github.com/vercel/next-forge) Turborepo for Next.js SaaS apps.**
...
2. Set `DATABASE_URL` in `packages/database/.env` (PostgreSQL; Neon recommended)
3. Run `bun run db:push` to apply the Prisma schema
4. Add optional integration keys (Clerk, Stripe, Resend, etc.) to the app `.env.local` files
5. Start all apps: `bun run dev`
...
All integrations besides the database are optional. Missing environment variables disable features instead of failing the app.

For detailed setup instructions, read the [documentation](https://www.next-forge.com/docs).

## Structure

This repo uses a monorepo structure managed by Turborepo:
